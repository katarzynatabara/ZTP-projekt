import listActions from 'modules/rezerwacja/list/rezerwacjaListActions';
...
const actions = {
  ...
  doDestroy: (id) => async (dispatch) => {
    try {
      ...
      await RezerwacjaService.destroyAll([id]);

      dispatch({
        type: actions.DESTROY_SUCCESS,
      });

      Message.success(i18n('entities.rezerwacja.destroy.success'));
  ...
  doDestroyAll: (ids) => async (dispatch) => {
    try {
      ...
      await RezerwacjaService.destroyAll(ids);
      ...
      Message.success(
        i18n('entities.rezerwacja.destroyAll.success'),
      );
