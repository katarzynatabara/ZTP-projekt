...
class RezerwacjaToolbar extends Component {
  doExport = () => {
    ...
    return (
      <Toolbar>
        ...
        {this.props.hasPermissionToAuditLogs && (
          <Link to="/audit-logs?entityNames=rezerwacja">
...
function select(state) {
  return {
    ...
    hasPermissionToCreate: rezerwacjaSelectors.selectPermissionToCreate(
      state,
    ),
    hasPermissionToImport: rezerwacjaSelectors.selectPermissionToImport(
      state,
    ),
  };
}

export default connect(select)(RezerwacjaToolbar);
